...
| <a name="input_maximum_elastic_worker_count"></a> [maximum\_elastic\_worker\_count](#input\_maximum\_elastic\_worker\_count) | The maximum number of elastic workers for the application service plan. | `number` | `null` | no |
...
| <a name="input_slots"></a> [slots](#input\_slots) | n/a | <pre>map(object({<br/>    app_settings        = optional(map(string), {})<br/>    sticky_app_settings = optional(list(string), [])<br/>  }))</pre> | `{}` | no |
...
| <a name="input_sticky_app_settings"></a> [sticky\_app\_settings](#input\_sticky\_app\_settings) | A list of app setting keys that should be marked as sticky (not overridden during deployment). | `list(string)` | `[]` | no |
