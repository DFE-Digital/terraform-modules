...
| Name | Type |
|------|------|
| [kubernetes_cron_job.main](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/cron_job) | resource |

## Inputs

| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_arguments"></a> [arguments](#input\_arguments) | list of required docker image arguments for k8s job to run | `list(string)` | <pre>[<br/>  "name",<br/>  "mount_path",<br/>  ""<br/>]</pre> | no |
| <a name="input_cm_mount"></a> [cm\_mount](#input\_cm\_mount) | config map to be mounted in terraform | `list(string)` | <pre>[<br/>  "name",<br/>  "mountpoint",<br/>  "configmap"<br/>]</pre> | no |
...
| <a name="input_enable_gcp_wif"></a> [enable\_gcp\_wif](#input\_enable\_gcp\_wif) | Let the job use the GCP workload identity federation service account to get a token | `string` | `false` | no |
...
| <a name="input_schedule"></a> [schedule](#input\_schedule) | job run schedule using cron syntax | `string` | `"1 0 * * *"` | no |
