...
```
which will enable logical replication for the primary database server.
This will cause the database server to restart several times.

Read replicas continue to use Azure's default physical replication configuration.

If a read replica is promoted during a disaster recovery event and logical replication is required, the logical replication settings must be manually re-applied to the promoted server.
