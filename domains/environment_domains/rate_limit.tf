resource "azurerm_cdn_frontdoor_firewall_policy" "rate_limit" {
  count = length(var.rate_limit) > 0 || var.rate_limit_max != null || var.allow_aks || var.block_ip || local.block_nonprod ? 1 : 0
  ...
  dynamic "custom_rule" {
    ...
    content {
      ...
      match_condition {
        ...
        operator       = "IPMatch"
        match_values   = ["20.117.102.231", "20.90.254.33", "172.167.85.211", "172.167.178.73"]
      }
    }
  }
